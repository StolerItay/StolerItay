...
async def gemini_materialize_placed_mass(
    ...
    _s2m_base = (
        ...
        "STRICT RULES:\n"
        "1. STYLE PRIORITY — The building's APPEARANCE (facade color, glass tint, cladding pattern, "
        "window grid, materials, lighting effects) must match Image 2 as closely as possible.\n"
        "2. SHAPE FROM IMAGE 1 ONLY — The building's SHAPE (tower count, heights, silhouette, "
        "podium form, crown) must come ENTIRELY from the white mass in Image 1. "
        "Image 2's building shape is IRRELEVANT — ignore it completely. "
        "If the white mass in Image 1 has 3 towers, your output MUST have exactly 3 towers. "
        "If it has 2, output 2. Never add or remove towers to match Image 2's building count.\n"
        "3. SELF-CHECK before finalizing: count the distinct white volumes in Image 1's mass, "
        "then verify your output has the exact same count and approximate silhouette. "
        "If they don't match, redo the building.\n"
        "4. BACKGROUND UNTOUCHED — Every pixel outside the building mass must be identical to Image 1.\n"
        "5. CAMERA FIXED — Same viewpoint, perspective, and framing as Image 1.\n"
        "6. LIGHTING CONSISTENT — The building's shadows and reflections must match the "
...
async def gemini_place_mass_in_scene(mass_path: Path, render_path: Path) -> tuple[Path, str]:
    ...
    _s1_base = (
        ...
        "the output.\n\n"
        "Image 2 = THE SCENE. This is the real-world photograph. The building currently visible in "
        "Image 2 is the OLD building being DEMOLISHED AND REPLACED. Do NOT use Image 2's building "
        "shape, height, proportions, or form — it is being removed. Take from Image 2 only: sky, "
        "roads, trees, surrounding buildings (not the one replaced), ground plane, lighting, camera.\n\n"
        "OUTPUT REQUIREMENTS:\n"
        "Produce Image 2's scene with the old building erased and replaced by a clean flat WHITE/"
        "LIGHT-GREY solid volume whose silhouette exactly matches the building in Image 1.\n\n"
        "STRICT RULES:\n"
        "1. GEOMETRY = Image 1 ONLY. Tower count, heights, width-to-height ratio, crown shape, "
        "podium — all from Image 1. Do not borrow any proportion from Image 2's building.\n"
        "2. BACKGROUND = Image 2 ONLY. Every pixel outside the new white mass must be identical "
        "to Image 2. Never use Image 1's background.\n"
        "3. WHITE MASS ONLY — flat featureless white or light-grey solid. No materials, no glass, "
        "no windows, no textures, no facade details.\n"
        "4. SCALE — the white mass base must sit on the correct ground plane of Image 2, sized "
...
async def judge_output(
    ...
    judge_prompt = (
        "You are a strict architectural visualization quality judge. "
        "Your job is to find flaws, not to be generous. Score hard.\n\n"
        "Image 1: the MASS MODEL — the ONLY source of truth for building geometry "
        "(exact tower count, relative heights, silhouette, taper angles, crown shape, podium form).\n"
        "Image 2: the REFERENCE RENDER — the target style only (materials, lighting, atmosphere).\n"
        "Image 3: the AI OUTPUT — what you are scoring.\n\n"
        "IMPORTANT TRAP TO AVOID: If Image 3 looks like a copy of Image 2's building style "
        "but does NOT match Image 1's geometry, geometry_accuracy must be LOW (1-4), "
        "not high — even if Image 3 looks photorealistic.\n\n"
        "Score on 4 criteria. Use the FULL 1–10 range. Be strict:\n"
        "1. geometry_accuracy (1–10): How precisely does Image 3 match Image 1's geometry?\n"
        "   - 10: Indistinguishable — exact tower count, identical relative heights, exact silhouette, "
        "exact crown form, exact podium shape. Zero visible deviation.\n"
        "   - 8-9: Correct count and approximate silhouette but minor taper/crown/podium deviations.\n"
        "   - 5-7: Correct tower count but noticeably wrong proportions, crown, or podium shape.\n"
        "   - 3-4: Wrong silhouette or significantly wrong proportions even if count is right.\n"
        "   - 1-2: Wrong tower count, or the building looks like it was copied from Image 2 "
        "rather than derived from Image 1's mass.\n"
        "2. style_match (1–10): How closely do Image 3's materials, facade, glass, lighting match Image 2?\n"
        "   - 10: Indistinguishable facade treatment, same glass color/tint, same cladding, same lighting.\n"
        "   - 1: Completely different materials/style.\n"
        "3. background_preservation (1–10): Is every pixel outside the building unchanged from Image 2?\n"
        "   - 10: Pixel-perfect preservation of sky, roads, trees, surrounding buildings.\n"
        "   - 1: Completely different background.\n"
        "4. overall (1–10): Holistic quality. A 10 requires BOTH geometry AND style to be excellent.\n\n"
